...
class AdvDA_GIN(object):
    def __init__(self,loader_source_train, 
        ...
        #Two pipelines
        #self.predict_label= Model(input_layer, class_output)
        #self.classify_domain = Model(input_layer,disc_output)
    ...
    def train(self):
        steps_src = self.loader_source_tr.steps_per_epoch

        for epoch in range(self.epochs):
            src_iter = iter(self.loader_source_tr)
            tgt_iter = iter(self.loader_target_tr)  

            for step in range(steps_src):
                source_batch, source_labels = next(src_iter)
                target_batch, target_labels = next(tgt_iter)
                

                self.train_batch(source_batch, source_labels,
                                 target_batch, target_labels, epoch)
