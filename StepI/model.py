...
class MaxDIrep(object):
    ...
    def log(self):
        ...
        f1s = []
        supports = []
        for P, R, supp in zip(self.precision_per_class,
                              self.recall_per_class,
                              self.support_per_class):
            p = P.result()
            r = R.result()
            f1s.append(2 * p * r / (p + r + eps))
            supports.append(supp.result())
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for (source_images, source_labels), (target_images, _) in zip(source_train_dataset, target_train_dataset):
...
class MaxDIrep_md(object):
    def __init__(self, x_source_train, y_source_train, 
        ...
        self.predict_label = tf.keras.Sequential([
          self.generator,
          self.classifier
        ])
