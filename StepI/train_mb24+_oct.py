...
# Anchor data/results paths to this script's own location (not the current
# working directory), so this runs the same whether invoked as
# `python StepI/train_mb24+_oct.py` from the repo root or `python
# train_mb24+_oct.py` from inside StepI/.
...
if __name__ == "__main__":
    ...
    label_path = DATA_DIR + "/labels/mb24/March/march_malware.csv"
    img_path = DATA_DIR + "/image_features/mb24/march/march"
    malware_march_x, malware_march_y = load_image_malware(img_path, label_path)

    label_path = DATA_DIR + "/labels/mb24/April/april_malware.csv"
    img_path = DATA_DIR + "/image_features/mb24/april/april"
    malware_april_x, malware_april_y = load_image_malware(img_path, label_path)

    label_path = DATA_DIR + "/labels/mb24/May/may_malware.csv"
    img_path = DATA_DIR + "/image_features/mb24/may/may"
    malware_may_x, malware_may_y = load_image_malware(img_path, label_path)

    label_path = DATA_DIR + "/labels/mb24/Sep/sep_malware.csv"
    img_path = DATA_DIR + "/image_features/mb24/sep/sep"
    malware_sep_x, malware_sep_y = load_image_malware(img_path, label_path)

    label_path = DATA_DIR + "/labels/mb24/Oct/oct_malware.csv"
    img_path = DATA_DIR + "/image_features/mb24/oct/oct"
    malware_oct_x, malware_oct_y = load_image_malware(img_path, label_path)

    # Load normal data source
    img_path = DATA_DIR + "/image_features/benign_source/dataset1"
    source_normal_x_1 = load_image_normal(img_path)
    source_normal_y_1 = np.zeros((source_normal_x_1.shape[0], 1))

    img_path = DATA_DIR + "/image_features/benign_source/dataset2"
    source_normal_x_2 = load_image_normal(img_path)
    source_normal_y_2 = np.zeros((source_normal_x_2.shape[0], 1))

    img_path = DATA_DIR + "/image_features/benign_source/dataset3"
    source_normal_x_3 = load_image_normal(img_path)
    source_normal_y_3 = np.zeros((source_normal_x_3.shape[0], 1))

    img_path = DATA_DIR + "/image_features/benign_source/dataset4"
    ...
    target_malware_train_x = malware_sep_x
    target_malware_train_y = malware_sep_y

    # Target malware test data
    target_malware_test_x = malware_oct_x
    target_malware_test_y = malware_oct_y
    ...
    print("  Target train = Sep malware+benign (labels discarded; used *unlabeled* for domain adaptation, see StepI/model.py)")
    print("  Target test  = Oct malware+benign (held out; this task's evaluation target)")
